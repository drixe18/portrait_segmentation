...
class SqueezeBlock(nn.Module):
    def __init__(self, exp_size, divide=4.0):
        ...
        if divide > 1:
            self.dense = nn.Sequential(
                ...
                nn.ReLU(int(exp_size / divide)), # PReLU
                nn.Linear(int(exp_size / divide), exp_size),
                nn.ReLU(exp_size), # PReLU
            )
        else:
            self.dense = nn.Sequential(
    ...
    def forward(self, x):
        ...
        out = torch.nn.functional.avg_pool2d(x, kernel_size=[int(height), int(width)]).view(batch, -1)
        out = self.dense(out)
...
class S2block(nn.Module):
    ...
    def __init__(self, nIn, nOut, config):
        ...
        if avgsize >1:
            ...
            self.up_res = nn.ConvTranspose2d(in_channels=nIn, out_channels=nIn, kernel_size=avgsize, stride=avgsize)
            # self.up_res = nn.Upsample(scale_factor=avgsize, mode='nearest')
...
class SINet_Encoder(nn.Module):
    ...
    def __init__(self, config,classes=20, p=5, q=3,  chnn=1.0):
        ...
        dim1 = 16
        dim2 = int(48 + 4 * (chnn - 1))
        dim3 = int(96 + 4 * (chnn - 1))
...
class SINet(nn.Module):
    ...
    def __init__(self,config, classes=20, p=2, q=3, chnn=1.0, encoderFile=None,):
        ...
        self.up = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=2, stride=2)
        # self.up1 = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=2, stride=2)
